...
> [!important]
>
> - 项目采用前后端分离开发，这是对应的[前端项目地址](https://github.com/caolib/vue3-vite)，或者试试这个AI做的前端[book-front-end](https://github.com/caolib/book-front-end)，界面更漂亮
...
1. 复制项目地址 `git@github.com:caolib/book_management_system.git`
...
> [!warning]
>
> 你必须自己先安装**MySQL**和**Redis**数据库，网上有很多安装教程
...
>
> 1. 主机：你的mysql数据库地址：
>    1. 如果安装在本地，直接填`localhost`
>    2. 如果安装在虚拟机上，可以先使用`ip addr`命令查看虚拟机地址，然后填写虚拟机地址（注意，如果虚拟机重启，这个地址大概率会变化）
>
...
redis**默认没有密码**，只需要填写主机号，同mysql，然后点击测试连接，成功后点击确定

![image-20241230124346981](https://s2.loli.net/2024/12/30/UsvGWhiOnxAdpuP.png)

#### 修改配置文件

打开项目下的`application-dev.yml`文件，修改两个数据的host（主机），端口号、用户名、密码等，修改后重启项目
...
打开`vite.config.js`文件,查看后端服务地址，后端默认是本地8080，如果无误则无需修改
...
打开控制台，运行`npm install`，如果报错，可以**以管理员方式打开控制台**再运行
...
默认运行在`http://localhost:5173`，登录账号和密码可以在mysql表中查看，或者注册一个账号
...
❓ 有问题可以在[Issues](https://github.com/caolib/book_management_system/issues)提问

⭐ 如果对你有帮助的话请点个star

☕ 请我[喝杯咖啡](https://bin-sites.pages.dev/sponsor/)

📧 联系我[caolibx@qq.com](mailto:caolibx@qq.com)
